...
/// FragmentThread() asynchronously and returns. After this point a fragment may terminate
/// either by cancellation via CancelPlanFragment(), or when FragmentThread() returns.
//
...
class FragmentMgr {
 ...
 private:
  ...
  /// Retrieves 'exec_state' corresponding to `fragment_id`. Calls exec_state->Prepare()
  /// and then exec_state->Exec(). Finally unregisters the exec_state from
  /// fragment_exec_state_map_. The exec_state must previously have been registered in
  /// fragment_exec_state_map_. Runs in the fragment's execution thread.
  ///
  /// TODO: Consider moving this to FragmentExecState to reduce complexity of sharing
  /// parameters and exec state with FragmentMgr.
  void FragmentThread(boost::shared_ptr<TExecPlanFragmentParams> params,
