...
class Coordinator {
 ...
 private:
  ...
  /// multiple threads. Obtains 'exec_state->lock()' prior to making the RPC, so that it
  /// serializes correctly with UpdateFragmentExecStatus().
  void ExecRemoteFragment(BackendExecState* exec_state);
