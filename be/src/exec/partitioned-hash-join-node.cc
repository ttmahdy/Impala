...
PartitionedHashJoinNode::PartitionedHashJoinNode(
  ...
  filters_ = tnode.runtime_filters;
}
  ...
  *built = false;
  ...
  do {
    ...
    for (int i = 0; i < num_rows; ++i) {
      ...
      if (BUILD_RUNTIME_FILTERS) {
        for (int j = 0; j < parent_->filters_.size(); ++j) {
          void* e = parent_->filter_expr_ctxs_[j]->GetValue(row);
          uint32_t h = RawValue::GetHashValue(e,
              parent_->filter_expr_ctxs_[j]->root()->type(), Bitmap::DefaultHashSeed());
          parent_->filter_bitmaps_[j]->Set<true>(h, true);
        }
      }
    }
...
bool PartitionedHashJoinNode::AllocateRuntimeFilters(RuntimeState* state) {
  ...
  for (int i = 0; i < filters_.size(); ++i) {
    filter_bitmaps_[i] = state->filter_bank()->AllocateScratchBitmap(
        state->filter_bank()->filter_bitmap_size());
  }
  return true;
}
...
bool PartitionedHashJoinNode::PublishRuntimeFilters(RuntimeState* state) {
  if (can_add_runtime_filters_) {
    ...
    for (int i = 0; i < filter_bitmaps_.size(); ++i) {
      if (filter_bitmaps_[i] == NULL) continue;
      state->filter_bank()->PublishBitmap(filters_[i].filter_id, filter_bitmaps_[i]);
      filter_bitmaps_[i] = NULL;
    }
    return true;
  }
  return false;
...
Status PartitionedHashJoinNode::BuildHashTables(RuntimeState* state) {
  ...
  if (input_partition_ == NULL && can_add_runtime_filters_) {
    ...
    // TODO: Use num_build_rows to estimate efficacy of Bloom filter, and disable filter
